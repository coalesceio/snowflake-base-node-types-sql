...
| `@accepted_values("<value>")` | ***(repeatable)*** Fails on rows whose value is outside the allow list.<br/>Repeat once per permitted value.<br/>Quote to match the column's data type — <br/> positive number/float/decimal/bool: `accepted_values("<num>")` or `accepted_values(<num>)` <br/> negative number/float/decimal: `accepted_values("-<num>")` <br/>string: `accepted_values("'<string>'")`.<br/>Examples:<br> string: `@accepted_values("'ALGERIA'")` <br/> positive numbers: `@accepted_values("1,2,3,4")` or `@accepted_values("1", "2", "3")` or `@accepted_values(1,2,3,4,5)` |
| `@rejected_values("<value>")` | ***(repeatable)*** Fails on rows whose value is in the deny list.<br/>Repeat once per forbidden value.<br/>Same quoting rules as `accepted_values`.<br/>Examples: <br/> string: `@rejected_values("'NA'")` <br/> positive numbers: `@accepted_values("1,2,3,4")` or `@accepted_values("1", "2", "3")` or `@accepted_values(1,2,3,4,5)` |
| `@min_max("<min>", "<max>")` | Fails on rows outside the inclusive range.<br/>Bounds are pasted into the SQL verbatim —<br/>number: `"0"` or `0`, <br/> negative number: `"-5"`,<br/>date: `"DATE '2026-01-01'"`.<br/>Examples: <br/> positive number: `@min_max(0, 4)` <br/> negative number: `@min_max("-10", "-5")` <br/> float/decimal number: `@min_max(55.8, 65.8)` |
| `@min_value("<min>")` | Fails on rows below the bound.<br/>Value formatting — see `min_max`.<br/>Examples: <br/> positive number: `@min_value(0)` <br/> negative number: `@min_value("-5")` <br/> float/decimal number: `@min_value(55.5)` |
| `@max_value("<max>")` | Fails on rows above the bound.<br/>Value formatting — see `min_max`.<br/>Examples: <br/> positive number: `@min_value(0)` <br/> negative number: `@min_value("-5")` <br/> float/decimal number: `@min_value(55.5)` |
