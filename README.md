...
* **Duplicate Business Keys with `upsert`**:  
If the SELECT/CTE returns more than one row per business key, the first load into an empty table inserts every duplicate without error, and later loads fail with "Duplicate row detected during DML action". De-duplicate upstream (e.g. `QUALIFY ROW_NUMBER() OVER (PARTITION BY <business key> ORDER BY <timestamp> DESC) = 1`).
